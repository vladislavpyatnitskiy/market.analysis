...
indices.plt <- function(us, rus, s=NULL, e=NULL){
  ...
  for (A in us){ U <- cbind(U, getData1(A, s, e)[,4]) } # Join data
  ...
  plot(DF[,1], ylim = c(min(DF), max(DF)),
       lty = 1, type = "l", lwd = 2,
       las = 1, xlab = "Trading Days", ylab = "Returns (%)",
  ...
  legend(x = "bottom", inset = c(0, -0.27), legend = colnames(DF), xpd = T,
         col = seq(ncol(DF)), lwd = 2, cex = .5, bty = "n", horiz = T)
