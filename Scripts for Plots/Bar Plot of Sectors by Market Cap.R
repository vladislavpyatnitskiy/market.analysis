...
finviz.sectors.marketcap <- function(x){ # Data Frame with info about sectors
  ...
  s <- read_html(sprintf("https://finviz.com/%s", x)) # HTML 
  
  tab <- s %>% html_nodes('table') %>% .[[8]] # Assign Table 
  
  y <- tab %>% html_nodes('tr') %>% html_nodes('td') %>% html_text()
  
  d <- NULL # Data Frame with Market Cap values
  ...
  B <- barplot(d, names.arg=names(d), horiz=T, las=1, xlim=c(mn, mx), xpd=F,
